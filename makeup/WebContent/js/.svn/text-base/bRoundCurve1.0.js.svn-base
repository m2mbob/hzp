...
function b_RoundCurves(classname,b_c,bg_c,state,tagname,titlebg,titleimg,bagy){
    ...
    for(var i=0;i<divs.length;i++){
        ...
        var imgPos2="background-position:-4px "+bagy+"px;";
        var imgPos3="background-position:-2px "+(bagy-1)*1+"px;";
        var imgPos4="background-position:-1px "+(bagy-2)*1+"px;";
        var conPos="background-position:left "+(bagy-1)*1+"px;";
        var imgPos5="background-position:-1px "+(bagy-36)*1+"px;";
        var imgPos6="background-position:-2px "+(bagy-36)*1+"px;";
        var imgPos7="background-position:-4px "+(bagy-36)*1+"px;";
        ...
        if(state==3){//如果是带标题方式，又可分为两种情况，一种直接用背景色，一种是用背景图片方式
            ...
            if(titleimg!=null){//标题带水平平铺的背景图片
                ...
                //标题栏样式
                objh3.style.background="url("+titleimg+") repeat-x left "+(bagy-4)+"px";
